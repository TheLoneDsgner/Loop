...
import { ChevronLeft, Check, Share, Plus, Info, Loader, Link } from "lucide-react";
...
const CreateList = () => {
    ...
    useEffect(() => {
        if (location.state && location.state.loopToEdit) {
            const { loopToEdit } = location.state;
            setListTitle(loopToEdit.title);
            setListDescription(loopToEdit.description);
            setListItems(loopToEdit.items);
            setListId(loopToEdit.id);
            setIsDirty(false); // When loading for edit, it's not dirty initially
            }
    ...
    const handleSaveList = () => {
        ...
        setTimeout(() => {
            ...
            if (listId === null) {
                // New list
                const newId = loops.length > 0 ? Math.max(...loops.map(list => list.id)) + 1 : 1
                listData.id = newId
                setLoops([...loops, listData])
                setListId(newId)
            } else {
                // Existing list
                const updatedLoops = loops.map(loop =>
                    loop.id === listId ? { ...loop, ...listData } : loop
                )
                        ...
                        <div className="header-buttons">
                            <div>
                                <Button
                                    ...
                                    disabled={listItems.length === 0 || isDirty}
                                >
                                    <Share size={20} /> Share
                                  </Button>
                            </div>
                            ...
                            <div className="add-items-form">
                                <TextField
                                    ...
                                    addItemBtn={
                                        ...
                                        >
                                            {editingItemIndex !== null ? <Check /> : <Plus />}
                                        </button>
                                    }
                                />
                            </div>
